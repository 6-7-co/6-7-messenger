...
### 4. Сборка установщика Windows (.exe)
...
Готовый установщик появится в `apps/desktop/release/`. Приложение по умолчанию ходит на `http://localhost:3000`; адрес сервера можно переопределить переменной окружения `MESSENGER_API_URL` перед сборкой.

### Как пользоваться вдвоём локально

1. Запустите сервер (`npm run dev:api`) на одной машине.
2. Откройте приложение и зарегистрируйте два аккаунта (например, `alex` и `bob`) — можно в двух окнах/профилях.
3. Введите никнейм друга в поле «Start chat by username…» — диалог создастся у обоих.
...
Соберите десктоп-приложение, указав адрес сервера:

```powershell
$env:MESSENGER_API_URL = "https://ваш-api.example.com"
npm run package:win -w @messenger/desktop
```

Раздайте установщик другу — и общайтесь онлайн.
